...
class ParticleSystem:
    def __init__(self):
        self.particulas = [] # [x, y, dx, dy, cor, ttl, size]
    ...
    def spawn(self, x, y, cor):
        for _ in range(80):
            ...
            self.particulas.append([x, y, math.cos(angle)*speed, math.sin(angle)*speed, cor, random.randint(30, 70), size])
    ...
    def update(self, frame):
        for p in self.particulas[:]:
            ...
            p[3] += 0.4  # Gravity
            p[5] -= 1
            alpha = max(0, min(1.0, p[5] / 30.0))
            c = tuple(int(v * alpha) for v in p[4])
            radius = int(p[6] * alpha)
            if radius > 0:
                cv2.circle(frame, (int(p[0]), int(p[1])), radius, c, -1)
            if p[5] <= 0: 
